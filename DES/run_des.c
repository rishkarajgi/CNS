...
int main(int argc, char* argv[]) {
	...
	if (argc < 2) {
		printf("Specify the action required\n");
		printf("1. -g : Key generation\n");
		printf("2. -e : Plain text encryption with key\n");
		printf("3. -d : Decryption of cipher text with key\n");
		return 1;
	}
	...
	if (strcmp(argv[1], ACTION_GENERATE_KEY) == 0) { // Generate key file
		...
		if (!key_file) {
			printf("Could not open file to write key. Are you sure you have required permissions ?");
			return 1;
		}
		...
		if (bytes_written != DES_KEY_SIZE) {
			printf("Error writing key to output file. Are you sure you have required permissions ?");
			fclose(key_file);
			free(des_key);
			return 1;
		}
		...
		if (strcmp(argv[1], ACTION_ENCRYPT) == 0) {
			...
			printf("Encrypting ! Please wait ...\n");
		} else {
			process_mode = DECRYPTION_MODE;
			printf("Decrypting Cipher text ! Please wait\n");
		}
